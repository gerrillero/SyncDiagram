...
function selectionchanged(args) {
    if (args.selectedItems.length) {
        ...
        if (args.selectedItems.length > 1) {
            ...
            diagram._selectedObject.lock = args.selectedItems.filter(function (e) { return !(args.selectedItems[0].constraints & (ej.datavisualization.Diagram.NodeConstraints.Delete | ej.datavisualization.Diagram.NodeConstraints.Drag | ej.datavisualization.Diagram.NodeConstraints.Resize)) }).length ? true : false;
        }
                    ...
                    if (handles[i].name === "Label") {
                        ...
                        if (args.selectedItems.length == 1 && type != "group" && args.selectedItems[0].type != "text" && args.selectedItems[0].labels && args.selectedItems[0].labels.length > 0) {
                            ...
                            for (var n = 0; n < items.length; n++) {
                                ...
                                if (child && child.labels && child.labels.length > 0) {
...
function textChanged(args) {
    diagram._modified = true;
    for (var i = 0; args && args.element && args.element.labels && i < args.element.labels.length; i++) {
        if (args.element.segments) {
...
function exportDiagram() {
    ...
    $("#exportformat").ejDropDownList({ selectedItems: [0] });
    $("#exportmode").ejDropDownList({ selectedItems: [0] });
