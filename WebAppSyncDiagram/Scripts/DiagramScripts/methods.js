    ...
    var LinkTool = (function (base) {
        ...
        LinkTool.prototype.mouseup = function (evt) {
            if (evt.target.id == "Line_shape") {
                ...
                this.svgHelper = null;
                base.prototype.mouseup.call(this, evt);
                this.diagram.update({ tool: ej.datavisualization.Diagram.Tool.SingleSelect | ej.datavisualization.Diagram.Tool.MultipleSelect });
                ...
                if (type !== "connector") {
                    ...
                    switch (prop) {
                        ...
                        case "borderWidth":
                            ...
                            if (Math.round(item.borderWidth) != option.borderWidth)
    ...
    if (type === "connector") {
        ...
        switch (prop) {
            case "LineStrokeColor":
                ...
                break;
            case "LineWidth":
                item.lineWidth = selectedObject.lineWidth = option.lineWidth = Number(String(args.text.trim()).substring(0, (args.text.trim()).length - 3));
                option.sourceDecorator = {};
                option.targetDecorator = {};
            ...
            case "Lock":
                var constraints;
                if (!selectedObject.lock)
                    constraints = ej.datavisualization.Diagram.ConnectorConstraints.Select | ej.datavisualization.Diagram.ConnectorConstraints.PointerEvents;
                else
...
function updateText(prop, item, args) {
    ...
    switch (prop) {
        ...
        case "Italic":
            ...
            break;
        case "fontSize":
            selectedObject.labelfontSize = option.fontSize = Number(args.text.trim());
            break;
        case "fontFamily":
...
function clearText() {
    ...
    if (document.getElementById("searchbox_icon-mini"))
        document.getElementById("searchbox_icon-mini").style.backgroundImage = 'url("../../Content/diagram/images/Search-Find.png")';
    if (document.getElementById("searchbox_icon-maxi"))
        document.getElementById("searchbox_icon-maxi").style.backgroundImage = 'url("../../Content/diagram/images/Search-Find.png")';
...
function onSearchBoxTextChange() {
    ...
    if (key && key == "") {
        ...
        if (document.getElementById("searchbox_icon-mini"))
            document.getElementById("searchbox_icon-mini").style.backgroundImage = 'url("../../Content/diagram/images/Close.png")';
    }
    setToolTip();
}
...
function onSearchBoxTextChange_maxi() {
    ...
    if (key.length > 0) {
        ...
        if (key == "") {
            ...
            document.getElementById("searchbox_icon-maxi").style.backgroundImage = 'url("../../Content/diagram/images/Close.png")';
        }
    }
...
function PropertyChangesFromPanel(args, prop) {
    if (!diagram._selectedObject.selected && diagram.selectionList[0]) {
        args.down = true;
        if (diagram.model.selectedItems.children.length > 1) { diagram.model.historyManager.startGroupAction(); }
        updateNodeProperties(args, prop);
        updateConnectorProperties(args, prop);
        if (diagram.model.selectedItems.children.length > 1) { diagram.model.historyManager.closeGroupAction(); }
    }
}
...
var SelectorVMClass = (function () {
    ...
    SelectorVMClass.prototype.EnableNodeProperties = function (item, isEnable, isMultipleSelection) {
        ...
        if (!isEnable) {
            ...
            if (item.type != "pseudoGroup") this.lock = (item.constraints & (ej.datavisualization.Diagram.NodeConstraints.Delete | ej.datavisualization.Diagram.NodeConstraints.Drag | ej.datavisualization.Diagram.NodeConstraints.Resize)) ? false : true;
...
function SetLabel(args) {
    if (args != null) {
        if (args == "fontsizediffer") {
            ...
            if (args.fontSize >= 8) {
                $("#fontsizelist").ejDropDownList("option", { selectedItems: [args.fontSize - 8] });
            }
            else {
        ...
        if (args == "fontFamilydiffer") {
            ...
            if (args.fontFamily == "Arial") {
                $("#fontStylelist").ejDropDownList("option", { selectedItems: [0] });
            }
            else if (args.fontFamily == "Aharoni") {
                $("#fontStylelist").ejDropDownList("option", { selectedItems: [1] });
            }
            else if (args.fontFamily == "Bell MT") {
                $("#fontStylelist").ejDropDownList("option", { selectedItems: [2] });
            }
            else if (args.fontFamily == "Fantasy") {
                $("#fontStylelist").ejDropDownList("option", { selectedItems: [3] });
            }
            else if (args.fontFamily == "Times New Roman") {
                $("#fontStylelist").ejDropDownList("option", { selectedItems: [4] });
            }
            else if (args.fontFamily == "Segoe UI") {
                $("#fontStylelist").ejDropDownList("option", { selectedItems: [5] });
            }
            else if (args.fontFamily == "Verdana") {
                $("#fontStylelist").ejDropDownList("option", { selectedItems: [6] });
            }
        }
    }
}
...
function setThickness(item, dropDownId) {
    if (item != null) {
        if (!item.segments)
            $("#" + dropDownId).ejDropDownList("option", { selectedItems: [Math.round(item.borderWidth - 1)] });
        else
            $("#" + dropDownId).ejDropDownList("option", { selectedItems: [Math.round(item.lineWidth - 1)] });
    }
    else {
